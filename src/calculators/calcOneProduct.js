...
function extractDataFromProduct(product) {
  ...
  if (product.cheat) {
    productData.cheat = product.cheat;
  }
}
...
function calculateAllTheStuffFinally(callback) {
  ...
  if (
    typeof productData.cheat === "undefined" ||
    (productData.cheat.amount === false &&
      productData.cheat.weight === false &&
      productData.cheat.price === false)
  ) {
    ...
    RESULTS.SUM_KG = productData.cheat.weight;
    RESULTS.SUM_AMOUNT = productData.cheat.amount;
    RESULTS.SALE_AVERAGE = productData.cheat.price;
  }

  RESULTS.SUM_KG = Math.round(100 * RESULTS.SUM_KG) / 100;
  RESULTS.FULL_SALE = Math.floor(RESULTS.SUM_KG * RESULTS.SALE_AVERAGE);
  RESULTS.COMMISSION = Math.floor(
    RESULTS.SUM_KG * RESULTS.SALE_AVERAGE * (productData.commission * 0.01)
  );
  RESULTS.OWNER_ERNINGS = Math.floor(
    RESULTS.FULL_SALE -
      productData.portage -
      productData.unload -
      RESULTS.COMMISSION -
